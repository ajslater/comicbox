...
def _reprint_to_cb(metron_reprint, primary_nid) -> dict:
...
def _alternative_name_to_cb(metron_alternative_name, primary_nid):
    comicbox_reprint = {}
    ...
    metron_id_attribute_to_cb(
        "reprint", metron_alternative_name, comicbox_reprint, primary_nid
    )
    return comicbox_reprint
...
def _reprints_to_cb(values):
    ...
    if metron_reprints := values.get(REPRINTS_TAGPATH):
        comicbox_reprints = [
            ...
            if (comicbox_reprint := _reprint_to_cb(metron_reprint, primary_nid))
        ]
    else:
        comicbox_reprints = []
        ...
        an_comicbox_reprints = [
            ...
            if (
                comicbox_reprint := _alternative_name_to_cb(
                    metron_alternative_name, primary_nid
                )
            )
        ]
...
def _reprint_from_cb(comicbox_reprint, primary_nid) -> dict:
    ...
    metron_id_attribute_from_cb(metron_reprint, comicbox_reprint, primary_nid)
    return metron_reprint
...
def _reprints_from_cb(values):
    ...
    return [
        ...
        if (metron_reprint := _reprint_from_cb(comicbox_reprint, primary_nid))
    ]
...
METRON_REPRINTS_TRANSFORM_FROM_CB = MetaSpec(
    key_map={REPRINTS_TAGPATH: (REPRINTS_KEY, PRIMARY_NID_KEYPATH)},
