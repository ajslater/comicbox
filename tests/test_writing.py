...
from .util import DUMP_CONFIG, get_tmp_dir, my_cleanup, my_setup, read_metadata

TMP_DIR = get_tmp_dir(__file__)
OLD_TEST_CBR_PATH = TMP_DIR / CBI_CBR_FN
NEW_TEST_CBZ_PATH = OLD_TEST_CBR_PATH.with_suffix(".cbz")
WRITE_CONFIG = Namespace(comicbox=Namespace(write=["cix"], timestamp=TEST_DATETIME))
...
def test_convert_to_cbz_and_cbi_to_cix():
    ...
    with Comicbox(OLD_TEST_CBR_PATH, config=WRITE_CONFIG) as car:
        car.add_source(md, ComicboxJsonSchema)
        # car._print_computed()
        car.write(dump_config=DUMP_CONFIG)

    # test
    read_metadata(NEW_TEST_CBZ_PATH, METADATA, READ_CONFIG_EMPTY)
