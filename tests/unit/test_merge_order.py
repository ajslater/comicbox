...
def test_merge_order_default_is_none() -> None:
    ...
    cfg = get_config()
    assert cfg.read.merge_order is None
...
def test_merge_order_explicit_list_resolves_to_enum_members() -> None:
    ...
    assert cfg.read.merge_order is not None
    assert cfg.read.merge_order[0] is MetadataSources.ARCHIVE_FILE
    assert cfg.read.merge_order[1] is MetadataSources.METRON_API
    assert cfg.read.merge_order[2] is MetadataSources.COMICVINE_API
    assert cfg.read.merge_order[3] is MetadataSources.CLI


def test_merge_order_appends_missing_members() -> None:
    """Members absent from a user list get appended at the end."""
    cfg = get_config({"comicbox": {"read": {"merge_order": ["ARCHIVE_FILE", "CLI"]}}})
    assert cfg.read.merge_order is not None
    # The two we listed first.
    assert cfg.read.merge_order[0] is MetadataSources.ARCHIVE_FILE
    assert cfg.read.merge_order[1] is MetadataSources.CLI
    # All MetadataSources members appear exactly once.
    assert set(cfg.read.merge_order) == set(MetadataSources)
    assert len(cfg.read.merge_order) == len(list(MetadataSources))


def test_merge_order_duplicate_raises() -> None:
    with pytest.raises(ValueError, match="duplicate"):
        get_config(
            {"comicbox": {"read": {"merge_order": ["ARCHIVE_FILE", "ARCHIVE_FILE"]}}}
        )
...
def test_merge_order_unknown_source_skipped() -> None:
    ...
    cfg = get_config(
        {"comicbox": {"read": {"merge_order": ["ARCHIVE_FILE", "MADE_UP_SOURCE"]}}}
    )
    assert cfg.read.merge_order is not None
    assert MetadataSources.ARCHIVE_FILE in cfg.read.merge_order
